...
class CollectoObject(GridObject):
    ...
    def handle_contact(self):
        ...
        if result!=-1:
            ...
            timr=time.time()-timr
            if timr>duration:
                self.level.backend.push_message("OVERTIME")
                result=False
            if result:
                if self.level.debug:
                    self.level.score.myscore=0
                    self.level.backend.push_message("Debug mode is ON, nullifying score")
                else:
                    self.level.score.myscore+=int((100.0/timr)+0.5)
                    self.level.backend.push_message(repr(self.level.score.myscore)+" total points, "+repr(self.level.score.mymoves)+" done, %.0f average points (point v10.1)"%(self.level.score.myscore/float(self.level.score.mymoves)))
    ...
    def question_test(self,duration):
        ...
        if do_sum:
            if not do_arc:
                ...
                ri=self.user_input_to_int(self.level.backend.slow_ask_question(("in %.2f seconds, "%duration)+repr(n)+"+"+repr(m)+"="))
                if ri!=ri: #i.e. is NaN
                    self.level.backend.push_message("I don't understand that answer (in figures, please).")
                    return -1
                elif ri!=n+m:
                    self.level.backend.push_message("wrong, it's "+str(n+m))
                    return False
                else:
                    self.level.backend.push_message("right")
                    return True
            else:
            ...
            if not do_arc:
                ...
                ri=self.user_input_to_int(self.level.backend.slow_ask_question(("in %.2f seconds, "%duration)+repr(n)+" times "+repr(m)+"="))
                if ri!=ri: #i.e. is NaN
                    self.level.backend.push_message("I don't understand that answer (in figures, please).")
                    return -1
                elif ri!=n*m:
                ...
                n*=m
                ri=self.user_input_to_int(self.level.backend.slow_ask_question(("in %.2f seconds, "%duration)+repr(n)+" divided by "+repr(m)+"="))
                if ri!=ri: #i.e. is NaN
                    self.level.backend.push_message("I don't understand that answer (in figures, please).")
                    return -1
                elif ri!=n//m: #Integer division!
                    self.level.backend.push_message("wrong, it's "+str(n//m)) #Integer division!
                    return False
                else:
                    self.level.backend.push_message("right")
                    return True
