...
class DumbLosInterface(SimpleInterface):
    ...
    def _fov_check(self,x,y,passthrough_once=True):
        if (not self.fov_status) or (self.level.debug_fov_off):
            return True,"ind"
        if not hasattr(self.level,"playerobj") or self.playerobj==None or self.playerobj.pt==None:
            return True,"ind"
        if ((x,y) in self._fov_cache) and not passthrough_once:
            return self._fov_cache[(x,y)],"nul"
        ...
        ret=-1
        typ="ind"
        if ret==-1 and (self.playerobj.pt in self._fov_typ_cache) and ((x,y) in self._fov_typ_cache[self.playerobj.pt]) and not passthrough_once:
            ret=True
            typ="nul"
        ...
        if not passthrough_once:
            ...
            if ret:
                if typ not in self._fov_typ_cache:
                    self._fov_typ_cache[typ]=[]
                if self.playerobj.pt not in self._fov_typ_cache:
                    self._fov_typ_cache[self.playerobj.pt]=[]
                self._fov_typ_cache[typ].append((x,y))
                self._fov_typ_cache[self.playerobj.pt].append((x,y))
        return ret,typ
    def redraw(self):
